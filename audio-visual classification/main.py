...
def train_epoch(args, epoch, model, device, dataloader, optimizer, scheduler, writer=None):
    ...
    for step, (spec, image, label) in enumerate(dataloader):
        ...
        std_s = std ** 2
        std_s = std_s.view(std_s.shape[0], -1)
        std_s = torch.mean(std_s, dim=1)
        # print(a_feature.shape,v_feature.shape)
        if epoch <= 5:
            index_a = ((p[:, 0]) * (1 - p[:, 1]))
            index_a = index_a.view(index_a.shape[0], -1)
            index_a = torch.squeeze(index_a, dim=1)
            std_a = torch.sum(std_s * index_a) / (torch.sum(index_a))
            # print(torch.sum(std_s * index_a), torch.sum(index_a))
            # print((std_s * index_a).shape)
            index_v = ((1 - p[:, 0]) * (p[:, 1]))
            index_v = index_v.view(index_v.shape[0], -1)
            index_v = torch.squeeze(index_v, dim=1)
            std_v = torch.sum(std_s * index_v) / (torch.sum(index_v))
            # print(std_a, std_v)
            # print(torch.sum(std_s * index_v), torch.sum(index_v))
            std_a_sum += std_a.item()
            std_v_sum += std_v.item()
            x_auxi = torch.tensor(0).float().cuda()
        else:

            auxi_loss = auxi_cross_entropy(auxi_out, label)
            x_visual = torch.sum(auxi_loss * ((1 - p[:, 0]) * (p[:, 1]))) / p.shape[0]
            x_audio = torch.sum(auxi_loss * ((p[:, 0]) * (1 - p[:, 1]))) / p.shape[0]
            if args.weak_modality == 'audio':
                x_auxi = x_audio
            else:
                x_auxi = x_visual
        # print(auxi_loss)
        ...
        loss_kl = torch.mean(loss_kl)
        # print(loss_kl)
        # print(loss_cls,x_visual,loss_kl)
        # print(x_auxi)

        loss = loss_cls + loss_kl * args.alpha + x_auxi * args.beta
    ...
    print(std_a_sum / step, std_v_sum / step)
    ...
    return _loss / len(dataloader), _loss_a / len(dataloader), _loss_v / len(dataloader), _a_diveristy / len(
        dataloader), _v_diveristy / len(dataloader), std_a_sum / step, std_v_sum / step
