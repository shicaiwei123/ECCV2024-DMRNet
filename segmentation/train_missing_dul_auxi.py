...
from src.build_model import build_model_dul_auxi
...
def parse_args():
    ...
    # args.last_ckpt="/home/icml//shicaiwei/ESANet-main/results/missing/cityscapes-with-depth/checkpoints_27_09_2022-11_54_16-757860/ckpt_latest.pth"
...
def train_main():
    ...
    model, device = build_model_dul_auxi(args, n_classes=n_classes_without_void)
    print('\n')
    print(args)
    print('\n')
    # checkpoint = torch.load("./results/full/cityscapes-with-depth/checkpoints_27_09_2022-11_57_21-523489/ckpt_latest.pth",
...
def train_one_epoch(model, train_loader, device, optimizer, loss_function_train,
    ...
    for i, sample in enumerate(train_loader):
        ...
        if modality == 'rgbd':
            pred_scales, fuse_cache, auxi_out, p, loss_kls = model(image, depth)
        elif modality == 'rgb':
            pred_scales, fuse_cache, auxi_out, p, loss_kls = model(image)
        else:
            pred_scales, fuse_cache, auxi_out, p, loss_kls = model(depth)

        # loss computation
        # print(len(pred_scales), len(target_scales))
        pred_scales = [pred_scales[ii] for ii in range(len(target_scales))]
        losses = loss_function_train(pred_scales, target_scales)
        loss_segmentation = sum(losses)

        loss_kls = torch.tensor(loss_kls)

        # auxi loss
        # p = (1 - p[:, 0]) * p[:, 1]
        p = (1 - p[:, 0]) + (1 - p[:, 1])
        index = [bool(ii) for ii in p]
        ...
        # print(target_scales_weak[0].shape)
        # print(p)
        # print(np.sum(p))

        print(target_scales_weak[0].shape[0])

        # 这里是index 可能会是0,也就是没有[0,1]的depth结果，这时候这个batch1的auxi loss 就是0
        ...
        if epoch > 10:
            ...
            if torch.isnan(loss_segmentation_weak).any():
                total_loss = loss_segmentation + 0.001 * torch.sum(loss_kls)
            else:

                total_loss = loss_segmentation + loss_segmentation_weak * 0.5 + 0.001 * torch.sum(loss_kls)

            if loss_segmentation == 0:  # 输出全是nan，没有计算loss
                continue
            ...
            if torch.isnan(total_loss):
                ...
                # print(loss_kls)
                # print(torch.sum(loss_kls))
                print(loss_segmentation.detach().cpu().numpy(),
                      loss_segmentation_weak.detach().cpu().numpy(), torch.sum(loss_kls))
                print('Loss is None')
                continue
        else:
    ...
    # stds = [x.cpu().detach().numpy() for x in stds]
    # print(stds)
