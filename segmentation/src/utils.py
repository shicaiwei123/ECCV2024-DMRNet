...
def print_log(epoch, local_count, count_inter, dataset_size, loss, loss_mmd,loss_logits, time_inter,
              learning_rates):
    ...
    print_string += '   loss_mmd: {:0.6f}'.format(loss_mmd.item())
    print_string += '   loss_logits: {:0.6f}'.format(loss_logits.item())
...
def print_log_original(epoch, local_count, count_inter, dataset_size, loss, time_inter,
              learning_rates):
    print_string = 'Train Epoch: {:>3} [{:>4}/{:>4} ({: 5.1f}%)]'.format(
        epoch, local_count, dataset_size,
        100. * local_count / dataset_size)
...
def save_ckpt(ckpt_dir, rgb_model, depth_model, optimizer, epoch):
    state = {
        'epoch': epoch,
...
def save_ckpt_every_epoch(ckpt_dir, rgb_model, depth_model, optimizer, epoch, best_miou,
                          best_miou_epoch):
    state = {
        'epoch': epoch,
        'rgb_state_dict': rgb_model.state_dict(),
...
def save_ckpt_one_modality(ckpt_dir, rgb_model, optimizer, epoch):
    state = {
        'epoch': epoch,
        'rgb_state_dict': rgb_model.state_dict(),
        'optimizer': optimizer.state_dict(),
...
def save_ckpt_every_epoch_one_modality(ckpt_dir, rgb_model, optimizer, epoch, best_miou,
                          best_miou_epoch):
    state = {
        'epoch': epoch,
        'rgb_state_dict': rgb_model.state_dict(),
...
def load_ckpt(rgb_model, depth_model, optimizer, model_file, device):
    if os.path.isfile(model_file):
        ...
        rgb_model.load_state_dict(checkpoint['rgb_state_dict'])
...
def load_ckpt_one_modality(rgb_model, optimizer, model_file, device):
    if os.path.isfile(model_file):
        print("=> loading checkpoint '{}'".format(model_file))
        if device.type == 'cuda':
            checkpoint = torch.load(model_file)
